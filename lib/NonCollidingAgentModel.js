...
//	drawsurface.style.border = "thick solid #0000FF"; //The border is mainly for debugging; okay to remove it
  ...
  function scale(row,maxRows = window.numRows){
    scale2 = Math.floor(row/23*maxRows)
    return(scale2)
  ...
  let rightPole = new NonCollidingArea('rightPole',Math.floor((7/23)*numRows), 4, grid,"images/pole1.png",bottomRight,Math.floor((10/23)*numRows),14);
  let leftPole = new NonCollidingArea('rightPole', Math.floor((7/23)*numRows), 4, grid,"images/pole2.png",bottomMiddle,scale(10),0);
  let cashier1 = new NonCollidingArea('rightPole', Math.floor((2/23)*numRows), 2, grid,"images/cashier3.png",bottomMiddle,scale(8),2);
